#!/usr/bin/env python3
"""
Train ConvLSTM precipitation nowcasting model.
...
from nowcasting.data import build_dataloaders
from nowcasting.metrics import compute_metrics, format_metrics_table
from nowcasting.model import ConvLSTMNowcast, count_parameters
from nowcasting.train import fit, load_checkpoint
from nowcasting.visualize import plot_prediction_example, plot_training_curve


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train ConvLSTM model for MRMS precipitation nowcasting."
    )
...
def resolve_path(path: str) -> str:
    ...
    Absolute paths, such as /scratch.global/..., are left unchanged.
    """
...
def main() -> None:
    ...
    train_loader, val_loader, test_loader, test_dataset = build_dataloaders(
        ...
        in_len=int(config["data"]["in_len"]),
        out_len=int(config["data"]["out_len"]),
    ...
    prediction_figure_path = resolve_path(config["outputs"]["prediction_figure_path"])
    metrics_path = resolve_path(config["outputs"]["metrics_path"])

    heavy_threshold = math.log1p(float(config["loss"]["heavy_threshold_mmhr"]))
    ...
    train_history, val_history, best_val_loss = fit(
        ...
        teacher_forcing_start=float(config["training"]["teacher_forcing_start"]),
        teacher_forcing_end=float(config["training"]["teacher_forcing_end"]),
        teacher_forcing_epochs=int(config["training"]["teacher_forcing_epochs"]),
        config=config,
    )
